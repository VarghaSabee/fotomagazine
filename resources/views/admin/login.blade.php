...
<div class="container">
    <h1 class="welcome text-center">Welcome Admin!</h1>
    <div class="card card-container">
        ...
        <form id="sign_in_adm" class="form-signin" method="POST" action="{{ route('admin.login') }}">
            {{ csrf_field() }}
                ...
                <input type="email" id="inputEmail" class="login_box" name="email" placeholder="Email Address" value="{{ old('email') }}" required autofocus>

                @if ($errors->has('email'))
                    <span class="text-danger"><strong>{{ $errors->first('email') }}</strong></span>
                @endif
            <p class="input_title">Password</p>
            <input type="password" id="inputPassword" class="login_box" name="password" placeholder="Password" required>
            <div class="">
